...
-- if architecture ~= "Lua 5.2" then table.insert(device, "You are using " .. architecture ..  " architecture. Take processor into your hands and switch it to \"Lua 5.2\", after that you will be able to install MineOS.") end
-- Проверяем, не планшет ли это
if component.isAvailable("tablet") then table.insert(device, "Tablet PC detected: you can't install MineOS on tablet because of primitive GPU and Screen.") end
-- Проверяем GPU
if component.gpu.maxResolution() < 150 then table.insert(device, "Bad GPU or Screen: MineOS requires Tier 3 GPU and Tier 3 Screen to give you full experience.") end
-- Проверяем оперативку
if math.floor(computer.totalMemory() / 1024 ) < 1024 then table.insert(device, "Not enough RAM: MineOS requires at least 1024 KB RAM.") end
-- Проверяем, не флоппи-диск ли это
if fs.get("/bin/edit.lua") == nil or fs.get("/bin/edit.lua").isReadOnly() then table.insert(device, "You can't install MineOS on floppy disk. Run \"install\" in command line and install OpenOS from floppy to HDD first. After that you're be able to install MineOS.") end

--Если нашло какое-то несоответствие сис. требованиям, то написать, что именно не так
if #device > 0 and not options.skipcheck then
  print(" ")
  for i = 1, #device do print(device[i]) end
  print(" ")
  return
else
...
applications = serialization.unserialize(getFromGitHubSafely(GitHubUserUrl .. "MrRagGame/MineOS/master/Applications.txt", "/MineOS/System/OS/Applications.txt"))
print(" ")
...
do
  ...
  getFromGitHubSafely(GitHubUserUrl .. "MrRagGame/MineOS/master/Installer/" .. _G.OSSettings.language .. ".lang", pathToLang)
  getFromGitHubSafely(GitHubUserUrl .. "MrRagGame/MineOS/master/MineOS/License/" .. _G.OSSettings.language .. ".txt", "/MineOS/System/OS/License.txt")
